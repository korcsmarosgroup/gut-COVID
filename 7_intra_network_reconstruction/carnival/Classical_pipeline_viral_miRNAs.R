...
all_genes_dea <- read.csv("input_files/expression_data/unfilt_degs_ileum_24h_infected_enterocytes_vs_mock.csv", sep = ",")
...
regulons <- read.csv("input_files/a_priori_networks/dorothea_abc_signed_directed_dev.txt", sep="") %>% 
  rename(tf = from) %>% 
    rename(target = to)
...
interactions <- read.csv("input_files/a_priori_networks/omnipath_signed_directed.txt", sep ="") 
...
host_viral_interactions <- 
  read.csv("input_files/miRNA_to_human_19.txt", sep = "\t")

##NOTE - we don't need this because here they are already gene names, not uniprot ####

#we convert from Uniprot to gene name (ADDITION - the script below did not work so I substituted it)

#library("org.Hs.eg.db") 
#host_viral_interactions$human_protein <- mapIds(org.Hs.eg.db, keys = host_viral_interactions$human_protein, keytype = "SYMBOL", column="UNIPROT") ##ADDED BY ME
...
host_viral_interactions_hgnc$hgnc_symbol <- host_viral_interactions_hgnc$human_protein
host_viral_interactions_hgnc$viral_protein <- host_viral_interactions_hgnc$viral_miRNA #to make it easier to proceed with the script
...
saveRDS(carnival_results_top50tf_pleitropic_minsize15, 
        file = "output_directory/networks/carnival_results_top50tf_pleitropic_minsize15.rds")

OutputCyto(carnival_results_top50tf_pleitropic_minsize15, 
           outputFile="output_directory/networks/carnival_results_top50tf_pleitropic_minsize15")
...
saveRDS(carnival_results_top50tf_pleitropic_minsize15_activation, 
        file = "output_directory/CARNIVAL/carnival_results_top50tf_pleitropic_minsize15_activation.rds")
OutputCyto(carnival_results_top50tf_pleitropic_minsize15_activation, 
           outputFile="output_directory/CARNIVAL/carnival_results_top50tf_pleitropic_minsize15_activation")
...
saveRDS(carnival_results_top50tf_pleitropic_minsize15_undefinedEffect, 
        file = "output_directory/networks/colon_new_08012021/colon_carnival_results_top50tf_pleitropic_minsize15_undefinedEffect.rds")
OutputCyto(carnival_results_top50tf_pleitropic_minsize15_undefinedEffect, 
           outputFile="output_directory/networks/colon_new_08012021/colon_carnival_results_top50tf_pleitropic_minsize15_undefinedEffect")
