...
ave_exp <- AverageExpression(colon, assays="RNA")
...
# Save
write.table(ave_exp$RNA, file = file.path(outdir,"expression","colon_average_expression_per_celltype.txt"), sep = "\t", quote = F,row.names = F)
...
# Set identities as condition_celltype
Idents(object=colon_inf) <- paste0(Idents(object=colon_inf), "_", colon$CellTypes, "_", colon$State)
...
# Save
write.table(ave_exp_inf$RNA, file = file.path(outdir, "expression","colon_infected_bystander_cells_average_expression_per_celltype.txt"), sep = "\t", quote = F,row.names = F)
...
get_degs <- function(data, ident1a, ident1b, ident2a, ident2b, out1, out2){
  ...
  for (i in unique(colon$CellTypes)){
    #print(i)
...
get_degs(colon, 'Colon_24h_', "", 'Colon_Mock_', "", "unfilt_degs_colon_24h_v_mock_all_cells.txt", "filt_degs_colon_24h_v_mock_all_cells_padj0.05_lfc0.5.txt")

# DEGS between 24h infected and mock
get_degs(colon_inf, 'Colon_24h_', "_Infected", 'Colon_Mock_', "_Non-Infected", "unfilt_degs_colon_24h_infected_v_mock_all_cells.txt", "filt_degs_colon_24h_infected_v_mock_all_cells_padj0.05_lfc0.5.txt")
