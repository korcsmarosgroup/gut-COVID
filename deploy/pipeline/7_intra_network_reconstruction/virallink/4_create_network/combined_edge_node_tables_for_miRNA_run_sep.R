...
rec_tf <- read.csv("output_directory/3_network_diffusion/TieDIE/tiedie.cn.sif", header=F, col.names=c("Source.node","Relationship","Target.node"), sep = "\t")

# Node heats from tiedie
heats <- read.csv("output_directory/3_network_diffusion/TieDIE/heats.NA", sep="=")

# Virus-receptor network
hbps <- read.csv("input_files/miRNA_to_human_proteins_uniprot.txt", sep = "\t")

# SARS-Cov2 gene symbols
#sars <- read.csv(args[4], sep = "\t")

# tf-deg network
tf_deg <- read.csv("output_directory/2_process_a_priori_networks/contextualised_regulator-deg_network.txt", sep = "\t")

# lfc table (unfiltered)
lfc <- read.csv("output_directory/1_process_expression_data/unfilt_degs_ileum_24h_infected_enterocytes_vs_mock.csv")
...
id_mapping <- AnnotationDbi::select(org.Hs.eg.db, keys=all_nodes$node, columns=c("UNIPROT","SYMBOL"), keytype="UNIPROT") 
...
all_nodes <- left_join(all_nodes, id_mapping, by = c("node"="UNIPROT"))
...
all_nodes$gene_symbol <- all_nodes$SYMBOL # I added this (Martina)
all_nodes <- all_nodes %>% dplyr::select(-c(SYMBOL))
