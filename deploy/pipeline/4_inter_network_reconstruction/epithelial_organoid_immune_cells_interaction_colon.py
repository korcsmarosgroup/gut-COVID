...
if os.path.isfile('epithelial_organoid_immune_cells_interaction_colon.log'):
    os.remove("epithelial_organoid_immune_cells_interaction_colon.log")

logging.basicConfig(filename = 'epithelial_organoid_immune_cells_interaction_colon.log', format = '%(asctime)s :: %(levelname)s :: %(funcName)s :: %(lineno)d :: %(message)s', level = logging.INFO)
...
def parse_args(args):
    ...
    parser.add_argument("-dfimm", "--df-imm",
                        help="<path to expression file> [mandatory]",
                        type=str,
                        dest="df_imm_file",
                        action="store",
                        required=True)

    parser.add_argument("-dfepi", "--df-epi-deg",
                        help="<path to filtedered expression file> [mandatory]",
                        type=str,
                        dest="df_epi_deg_file",
                        action="store",
                        required=True)
    ...
    return results.df_imm_file, results.df_epi_deg_file, results.lr, results.output_file

logging.info("epithelial_organoid_immune_cells_interaction_colon.py is starting")
df_imm_file, df_epi_deg_file, lr, output_file = parse_args(sys.argv[1:])

# Importing expression information from healthy and diseased conditions in different cells as pandas dataframes
df_imm = pd.read_csv(df_imm_file, index_col = 0) # contains all of the cells and conditions

df_epi_deg = pd.read_csv(df_epi_deg_file, index_col=5, sep = "\t") # modify it if the parameters are changing
...
if "bystander" in df_epi_deg_file:
...
if "infected" in df_epi_deg_file:
...
logging.info("Set up immune cell expression")
...
logging.info("Going through each cell pair")
...
logging.info("Sort dataframe for cell type - remove replicates")
...
logging.info("Writing out all interactions in all conditions")
# writing out all interactions in all conditions
epi_imm_interactions.to_csv(output_file, sep='\t', index=False)

logging.info("epithelial_organoid_immune_cells_interaction_colon.py is successfully finished!")
