...
message("\nStarting differential expression analysis script: barplot_num_ints_per_receptor_num_imm_cells.R\n")
...
for (name in names(ints_spl)){
  ...
  # Save
  ggsave(plot_ligs, file=file.path(paste0(plot_out,file_n)),width = 30, height=8) 
  
  ##### Plot together #####
  
  file_n_sep <- paste0("together_num_ints_per_receptor_num_imm_cells_", name, ".png")
  
  plot_non_facet(ints_tog, file.path(paste0(plot_out, file_n_sep)),10, 3, 5, TRUE)
  ...
  for (cell in cell_types) {
    ...
    # plot
    plot_non_facet(ints_sep_cell, file.path(paste0(plot_out, file_p)),8, 3, 4, TRUE)
  }
