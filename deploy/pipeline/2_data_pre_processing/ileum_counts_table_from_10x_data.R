...
message("\nStarting differential expression analysis script: ileum_counts_table_from_10x_data.R\n")
...
ident <- paste0(Idents(object=Illeum_H_T), "_", Illeum_H_T$CellTypes)
generate_average_expression(seu_object = Illeum_H_T,
                            output_file_path = file.path(outdir, "expression", "ileum_average_expression_per_celltype_test.txt"),
                            ident=ident)


##### Get counts table infected cells vs uninfected cells #####

ident <- paste0(Idents(object=Illeum_H_T), "_", Illeum_H_T$CellTypes, "_", Illeum_H_T$State)
generate_average_expression(seu_object = Illeum_H_T,
                            output_file_path = file.path(outdir, "expression", "ileum_infected_bystander_cells_average_expression_per_celltype_test.txt"),
                            ident=ident)
...
get_degs <- function(data, ident1a, ident1b, ident2a, ident2b, out1, out2){
  ...
  for (i in unique(Illeum_H_T$CellTypes)){
    #print(i)
...
Idents(object=Illeum_H_T) <- Illeum_H_T$orig.ident
Idents(object=Illeum_H_T) <- paste0(Idents(object=Illeum_H_T), "_", Illeum_H_T$CellTypes)
get_degs(Illeum_H_T, 'Illeum_24h_', "", 'Illeum_Mock_', "", "unfilt_degs_ileum_24h_v_mock_all_cells.txt", "filt_degs_ileum_24h_v_mock_all_cells_padj0.05_lfc0.5.txt")

# DEGS between 24h infected and mock
Idents(object=Illeum_H_T) <- Illeum_H_T$orig.ident
Idents(object=Illeum_H_T) <- paste0(Idents(object=Illeum_H_T), "_", Illeum_H_T$CellTypes, "_", Illeum_H_T$State)
get_degs(Illeum_H_T, 'Illeum_24h_', "_Infected", 'Illeum_Mock_', "_Non-Infected", "unfilt_degs_ileum_24h_infected_v_mock_all_cells.txt", "filt_degs_ileum_24h_infected_v_mock_all_cells_padj0.05_lfc0.5.txt")

# DEGS between 24h bystander and mock
get_degs(Illeum_H_T, 'Illeum_24h_', "_Bystander", 'Illeum_Mock_', "_Non-Infected", "unfilt_degs_ileum_24h_bystander_v_mock_all_cells.txt", "filt_degs_ileum_24h_bystander_v_mock_all_cells_padj0.05_lfc0.5.txt")
