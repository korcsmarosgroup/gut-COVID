...
message("\nStarting differential expression analysis script: Functional_analysis_merged_networks.R\n")
...
backgr_ppi <- read.csv(args[1], sep = "")  #COLON #run once for ileum and once for colon
...
write.table(go2_df, args[3], quote = FALSE, row.names = FALSE, sep = "\t")
...
write.table(re1_df, args[4], quote = FALSE, row.names = FALSE, sep = "\t")
