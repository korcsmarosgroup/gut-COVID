...
# Output directory
outdir <- args[5]
#outdir <- "/home/gutcovid/8_intra_output_data/virallink/ileum_mirna"
...
dorothea <- args[2]
omnipath <- args[3]
#expressed <- read.csv("/home/gutcovid/1_input_data/intracellular_networks/expression_data/expressed_genes_ileum_infected_enterocytes.txt", sep = "\t")
#dorothea <- "/home/gutcovid/1_input_data/intracellular_networks/a_priori_networks/dorothea_abc_signed_directed_dev.txt"
#omnipath <- "/home/gutcovid/1_input_data/intracellular_networks/a_priori_networks/omnipath_signed_directed.txt"
...
for (i in files){
  ...
  network_f <- network %>% dplyr::filter((get(source_col) %in% expressed$Gene) & (get(target_col) %in% expressed$Gene))
