...
lr_sep<- read.csv("results/triana_2020/ligand_receptor_ints/3rd_set_imm_cells/ileum/infected/network_figs/ligand_receptor_immcell/imm_enterocytes_2/edge_tables/ileum_infec_imm_entero2_ligand_imm_cell_network.txt", sep = "\t")
...
out_f <- "results/triana_2020/ligand_receptor_ints/3rd_set_imm_cells/ileum/infected/plots/ligands_v_immcells_10032021/"
#out_f <- "results/triana_2020/ligand_receptor_ints/3rd_set_imm_cells/colon/infected/plots/ligands_v_immcells_10032021/"
...
hplot <- function(indata2, out_f){
  ...
  for (name1 in names(indata_spl)){
   ...
   # bk2 <- c(seq(0.5,85, by = 0.5))
    ...
    out_p <- paste0(out_f, "ileum_heatplot_ligand_vs_immcell_", ligand_celltype, "_", name1, ".png")
