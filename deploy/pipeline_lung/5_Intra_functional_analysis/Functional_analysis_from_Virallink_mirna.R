...
# Save dot plot
filep <- file.path("GO_overrep_mirna.pdf")
pdf(filep)
print(dot_plot)
dev.off()

write.table(go2_df, "GO_functional_merged_network_mirna.txt", quote = FALSE, row.names = FALSE, sep = "\t")
...
# Save dot plot
filep2 <- file.path("REACT_overrep_mirna.pdf")
print(dot_plot2)
pdf(filep2, width=6.5)
dev.off()
  
write.table(re1_df, "REACT_functional_merged_network_mirna.txt", quote = FALSE, row.names = FALSE, sep = "\t")
