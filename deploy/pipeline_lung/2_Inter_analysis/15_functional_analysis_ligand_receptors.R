...
message("\nStarting differential expression analysis script: 15_functional_analysis_ligand_receptors.R\n")
...
backgr_lr_ints <- read.csv(args[2], sep = "\t")
...
for (name in names(lr_ints_spl)){
  ...
  for (cell_name in names(ints_cell)){
    ...
    func_analysis(rec_ligs, backgr_both, paste0("recs+ligs_", cell_name, "_",name), file.path(out_fold))
    
    # Ligands only
    ligs <- unique(unlist(ints_tog$ligand))
    all_ligs <- c(all_ligs, ligs)
    # Call functional analysis
    #func_analysis(ligs, backgr_ligs, paste0("ligs_", cell_name, "_",name), file.path(out_fold,"ligands")) #not run for the moment
    
    # Receptors only
    recs <- unique(unlist(ints_tog$receptor))
    all_recs <- c(all_recs, recs)
    # Call functional analysis
    #func_analysis(recs, backgr_recs, paste0("recs_", cell_name, "_",name), file.path(out_fold,"receptors")) #not run for the moment
  }
  ...
  func_analysis(all_recs_ligs, backgr_both, paste0("recs+ligs_all_",name), file.path(out_fold))
